...
import { Film, BarChart2, TrendingUp, Star, ChevronLeft, ChevronRight, Menu } from "lucide-react";
    ...
    return (
        <>
            <button
                ...
                className="fixed top-4 left-4 z-[60] p-2 bg-[#1F1F1F] text-white rounded-lg md:hidden hover:bg-gray-800 transition-colors"
            >
                <Menu size={24} />
            </button>
            ...
            >
                <div className="p-6 flex items-center justify-between h-20 border-b border-gray-800/50">
                    <div className={`flex items-center gap-3 ${!isOpen && "justify-center w-full"}`}>
                        <div className="w-8 h-8 bg-[#F5C518] rounded-lg flex items-center justify-center flex-shrink-0">
                ...
                <nav className="flex-1 py-6 px-3 space-y-2 overflow-y-auto">
                    {navItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                <item.icon
                                    className={`w-5 h-5 flex-shrink-0 transition-colors ${isActive ? "text-black" : "text-gray-400 group-hover:text-white"
                                        }`}
                                />
                ...
                <div className="p-4 border-t border-gray-800/50">
                    <button
                        onClick={() => setIsOpen(!isOpen)}
                        className={`w-full flex items-center gap-3 p-3 rounded-xl text-gray-400 hover:bg-gray-800 hover:text-white transition-all ${!isOpen && "justify-center"
                            }`}
                    >
